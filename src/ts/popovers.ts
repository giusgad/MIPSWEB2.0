...
export async function showFilePopover(id: string, toggleButton: HTMLElement) {
    ...
    window.addEventListener('click', (event) => {
        ...
        const toggleButton = activeFileContainer?.getElementsByClassName('toggle-button')[0];
        if (popover && activeFileContainer && toggleButton) {
            if (!popover.contains(event.target as Node) && !toggleButton.contains(event.target as Node)) {
                hideFilePopover();
                document.removeEventListener('click', () => {});
            }
        }
    });
    document.getElementById('all-files')?.addEventListener('scroll', (event) => {
        hideFilePopover();
    });
    await render('popover', '/app/popovers/file-popover.ejs', { file: getFile(parseInt(id)) });
}

export async function showPopover(rect: DOMRect) {
