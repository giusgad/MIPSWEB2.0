...
(window as any).showFilePopoverOnClick = async function(id: string, toggleButton: HTMLElement) {
    await showFilePopover(id, toggleButton);
};

(window as any).showFormOnClick = async function (form: string, dataString: string) {
...
(window as any).changeFileOnClick = async function(stringFileId: string, fileTab: HTMLElement) {
    let fileTabs = fileTab.parentElement;
    let scrollLeft = undefined;
    if (fileTabs) { scrollLeft = fileTabs.scrollLeft; }
    const fileId = parseInt(stringFileId);
    if (isNaN(fileId)) console.error('Invalid fileId');
    await changeFile(fileId);
    if (scrollLeft) { (document.getElementById('files-tabs') as HTMLElement).scrollLeft = scrollLeft; }
};

(window as any).openFileOnDbClick = async function(stringFileId: string, fileContainer: HTMLElement) {
    let allFiles = fileContainer.parentElement;
    let scrollTop = undefined;
    if (allFiles) { scrollTop = allFiles.scrollTop; }
    const fileId = parseInt(stringFileId);
    if (isNaN(fileId)) console.error('Invalid fileId');
    await openFile(fileId);
    if (scrollTop) { (document.getElementById('all-files') as HTMLElement).scrollTop = scrollTop; }
};

(window as any).closeFileOnClick = async function(stringFileId: string, button: HTMLElement) {
    const fileTab = button.parentElement?.parentElement;
    let filesTabs = fileTab?.parentElement;
    let scrollLeft = undefined;
    if (filesTabs) { scrollLeft = filesTabs.scrollLeft; }
...
(window as any).renameFileOnClick = async function(stringFileId: string) {
    const fileId = parseInt(stringFileId);
    const newFileName = (document.getElementById('new-file-name') as HTMLInputElement)?.value;
    try {
