...
export function addLoader(id: string) {
    const loaders: string[] = getFromStorage("local", "loaders") || [];
    if (!loaders.includes(id)) loaders.push(id);
...
export function removeLoader(id: string) {
    const loaders: string[] = getFromStorage("local", "loaders") || [];
