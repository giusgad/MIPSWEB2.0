...
export abstract class Instruction {
    ...
    constructor(symbol: string, params: string, format: string, opcode: Binary, funct: Binary | undefined, description: string) {
        this.symbol = symbol;
        this.params = params;
        this.format = format;
        this.opcode = opcode;
    ...
    basic(params: { [key: string]: Binary }): string {
        ...
        for (const name of paramsNames) {
            if (name.includes('(') && name.includes(')')) {
                ...
                const offsetValue = params[offsetName]?.getValue() || 0;
